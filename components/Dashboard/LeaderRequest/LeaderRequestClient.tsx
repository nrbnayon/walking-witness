...
export function LeaderRequestClient({ onApproveAll, onDeclineAll }: LeaderRequestClientProps) {
  ...
  const [pendingAction, setPendingAction] = useState<{ id: string, type: 'Approve' | 'Decline' } | null>(null);

  // Confirmation state for bulk actions
  const [confirmBulkOpen, setConfirmBulkOpen] = useState(false);
  const [pendingBulkAction, setPendingBulkAction] = useState<'ApproveAll' | 'DeclineAll' | null>(null);
  ...
  const initiateAction = (e: React.MouseEvent, type: 'Approve' | 'Decline', id: string) => {
    e.stopPropagation();
  ...
  const handleConfirmBulkAction = () => {
    ...
    try {
      if (pendingBulkAction === 'ApproveAll') {
        ...
        setRequests(requests.map(r => ({ ...r, status: 'Approved' as RequestStatus })));
        ...
        const pendingCount = requests.filter(r => r.status === 'Pending').length;
        setRequests(requests.map(r => ({ ...r, status: 'Declined' as RequestStatus })));
  ...
  return (
    <>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-4 font-medium" onClick={(e) => e.stopPropagation()}>
                          {item.status !== 'Approved' && (
                            <button 
                              onClick={(e) => initiateAction(e, "Approve", item.id)}
                          ...
                          {item.status !== 'Declined' && (
                            <button 
                              onClick={(e) => initiateAction(e, "Decline", item.id)}
      ...
      <ConfirmationModal
        ...
        title={pendingBulkAction === 'ApproveAll' ? 'Approve All Requests?' : 'Decline All Requests?'}
        message={`Are you sure you want to ${pendingBulkAction === 'ApproveAll' ? 'approve' : 'decline'} all leader requests? This action can be reversed, but notifies permissions.`}
        confirmText={pendingBulkAction === 'ApproveAll' ? 'Approve All' : 'Decline All'}
