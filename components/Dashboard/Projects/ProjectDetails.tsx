...
type SectionKey = 'basic' | 'details' | 'updates' | 'category' | 'stories' | 'impact' | 'pastorSupport' | 'livestock' | 'other';
...
export function ProjectDetails({ id }: { id: string }) {
  ...
  const validate = () => {
    ...
    if (!formData.sponsor.trim()) newErrors.sponsor = "Sponsor name is required";
    if (!formData.established.trim()) newErrors.established = "Establishment date is required";
  ...
  const updateLivestock = (index: number, field: 'name' | 'count', value: string | number) => {
  ...
  const updateArrayField = (field: 'pastorSupport' | 'other', index: number, value: string) => {
  ...
  const SectionHeader = ({ title, section }: { title: string, section: SectionKey }) => (
    <div className="flex items-center justify-between mb-4">
      <h3 className="font-semibold text-primary dark:text-gray-100">{title}</h3>
      {!isNew && (
        <button 
  ...
  const DisplayText = ({ value, placeholder = "-" }: { value?: string | number, placeholder?: string }) => (
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
        <span className="cursor-pointer hover:text-gray-900" onClick={() => router.push('/projects')}>Projects</span>
        <ChevronRight className="h-4 w-4" />
        <span className="font-medium text-primary dark:text-gray-100">{isNew ? 'New Project' : 'Project details'}</span>
      </div>
                ...
                {isEditing('basic') ? (
                  <>
                    <Input 
                      ...
                      onChange={(e) => {
                        setFormData({...formData, village: e.target.value, projectName: e.target.value});
                        if (errors.village) setErrors({...errors, village: ""});
                      }}
                      placeholder="e.g. Kirembe Park View"
                      className={cn(errors.village && "border-red-500 focus-visible:ring-red-500")}
                    />
                 ...
                 {isEditing('basic') ? (
                  <>
                    <Input 
                      ...
                      onChange={(e) => {
                        setFormData({...formData, pastor: e.target.value, leader: e.target.value});
                        if (errors.pastor) setErrors({...errors, pastor: ""});
                      }}
                      placeholder="Pastor name"
                      className={cn(errors.pastor && "border-red-500 focus-visible:ring-red-500")}
                    />
                  ...
                  <>
                    <Input 
                      ...
                      onChange={(e) => {
                        ...
                        if (errors.sponsor) setErrors({...errors, sponsor: ""});
                      }}
                      placeholder="Sponsor name"
                      className={cn(errors.sponsor && "border-red-500 focus-visible:ring-red-500")}
                    />
                  ...
                  <>
                    <Input 
                      ...
                      onChange={(e) => {
                        ...
                        if (errors.established) setErrors({...errors, established: ""});
                      }}
                      placeholder="Date established"
                      className={cn(errors.established && "border-red-500 focus-visible:ring-red-500")}
                    />
                ...
                <>
                  <textarea 
                    className={cn(
                      ...
                      errors.details && "border-red-500 focus-visible:ring-red-500"
                    )}
                ...
                <>
                  <textarea 
                    className={cn(
                      ...
                      errors.stories && "border-red-500 focus-visible:ring-red-500"
                    )}
              ...
              {isEditing('impact') ? (
                <>
                  <textarea 
                    className={cn(
                      "flex min-h-[100px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 resize-y",
                      errors.impact && "border-red-500 focus-visible:ring-red-500"
                    )}
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
           ...
           {isEditing('pastorSupport') ? (
             <Button variant="ghost" size="sm" onClick={() => addArrayItem('pastorSupport')}>
                ...
                {isEditing('pastorSupport') ? (
                   <>
                    <Input 
                      ...
                      placeholder="Amount"
                    />
                    <Button variant="ghost" size="icon" onClick={() => removeArrayItem('pastorSupport', idx)} className="text-red-500 hover:text-red-700">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
        ...
        <div className="space-y-4">
          <label className="text-xs font-medium text-gray-500">Set name & count</label>
            ...
            <div key={idx} className="flex gap-4 items-center">
              {isEditing('livestock') ? (
                <>
                  <Input 
                    value={item.name}
                    onChange={(e) => updateLivestock(idx, 'name', e.target.value)}
                  ...
                  <Input 
                    type="number"
                    value={item.count}
                    onChange={(e) => updateLivestock(idx, 'count', parseInt(e.target.value))}
                    placeholder="Count"
                    className="w-32"
                  />
                  <Button variant="ghost" size="icon" onClick={() => removeLivestock(idx)} className="text-red-500 hover:text-red-700">
                    <Trash2 className="h-4 w-4" />
                  </Button>
                </>
              ) : (
                <div className="flex gap-4 items-center w-full bg-gray-50 dark:bg-gray-700/50 rounded-md p-2">
                   ...
                   <div className="w-24 text-sm text-gray-500">Qty: {item.count}</div>
                </div>
              )}
            </div>
          ))}
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
           ...
           {isEditing('other') ? (
             <Button variant="ghost" size="sm" onClick={() => addArrayItem('other')}>
               <Plus className="h-4 w-4 mr-1" /> Add
             </Button>
           ) : (
             !isNew && (
                 ...
                 {isEditing('other') ? (
                    <>
                      <Input 
                        ...
                        placeholder="Amount"
                      />
                      <Button variant="ghost" size="icon" onClick={() => removeArrayItem('other', idx)} className="text-red-500 hover:text-red-700">
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    </>
                 ) : (
